...
def run_monitoring_check():
    ...
    with app.app_context():
        ...
        try:
            crop_areas = json.loads(config.crop_areas)
        except json.JSONDecodeError:
            print("错误：裁剪区域JSON格式不正确，已忽略。")
            crop_areas = {}
        ...
        chrome_options.add_argument(f'--window-size={config.screenshot_width},1080') # 初始尺寸
        
        driver = webdriver.Chrome(options=chrome_options)
        SCREENSHOT_DIR = "/app/screenshots" # 在容器内的固定路径
...
def settings_page():
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        flash('设置已成功保存！后台任务将按新间隔重启。')
        
        # 使用新的间隔时间重新调度任务
        if scheduler.get_job('monitoring_job'):
            scheduler.reschedule_job('monitoring_job', trigger='interval', seconds=config.monitor_interval_seconds)
        else: # 如果任务不存在，则添加
            scheduler.add_job(id='monitoring_job', func=run_monitoring_check, trigger='interval', seconds=config.monitor_interval_seconds)
